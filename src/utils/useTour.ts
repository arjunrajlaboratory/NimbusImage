...
export function useTour() {
  ...
  if (!manager) {
    throw new Error("TourManager not provided. Did you forget app.provide('tourManager', tourManager)?");
  }
  return {
    startTour: (tourName: string): Promise<void> => manager.startTour(tourName),
    loadAllTours: (): Promise<Record<string, ITourMetadata>> => manager.loadAllTours(),
    isTourActive: (): boolean => manager.isTourActive(),
    nextStep: (targetElementId?: string): Promise<void> => manager.nextStep(targetElementId),
  };
}
