...
| 15 | Medium | `CellTableCard.vue` `stalenessText` | Round 3 (live, 2026-09-25): without stored digests staleness compares membership only, so it sees added and removed cells but never a moved vertex. The `hasGeometryHashes` caveat was appended to the stale branch only, so the up-to-date branch asserted "Up to date with the cell polygons." — a claim about geometry the check cannot make. Live on the lymph node table (`hasGeometryHashes: false`, `upToDate: true`) it said exactly that. The symmetric twin of an earlier fix; no test covered the combination. | fixed — the up-to-date branch now reads "No cells added or removed since this table was built" plus the same caveat, pinned in *"does not claim the polygons match when it cannot see edits"*. |
| 16 | Nit | `RecomputeTableDialog.vue` | Round 3 (live, 2026-09-25): the dialog's own prose still read "also update **neighbours** in the cells' previous locations" — the one British spelling the US-spelling sweep left in this branch's user-facing text. It survived a follow-up audit too, because that grep searched for `neighbourhood`, not `neighbour`. | fixed — "neighbors", verified in the running dialog. **When a sweep reports clean, check what the pattern structurally cannot match.** |
