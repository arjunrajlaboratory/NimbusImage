...
| 7 | Low | `TranscriptOverlay.vue` `showDensity` | Several genes share one heat map drawn white (summed counts belong to no gene). | by-design — one OSM layer per gene would be up to 8 tile layers; revisit if users ask for per-gene heat maps. |
...
| 10 | Deferred | plan §12.3 | Opacity control. | deferred — fill opacity is fixed at 0.85; add when someone needs it. |
