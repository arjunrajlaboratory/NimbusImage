...
| 7 | Low | `TranscriptOverlay.vue` `showDensity` | Several genes shared one heat map drawn white. | fixed ("fix everything" round) — one OSM layer per gene in its color, hidden when the gene is removed; *"draws one heat map per gene in its own color and hides removed ones"*. |
