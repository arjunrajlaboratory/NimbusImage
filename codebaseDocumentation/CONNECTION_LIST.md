...
- [ ] **The failure warning keys off uncovered displayed members, not off any request's fate.** An obsolete request (for tracks shown before a scope change) can fail after a newer one already covered everything displayed; flagging that failure strands a warning Retry cannot clear (it finds nothing missing and returns early — which must itself clear a moot flag). — *"clears a stale failure once every displayed member is covered"*
