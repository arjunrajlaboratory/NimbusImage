...
- [ ] **The selection action panels clear the Timelapse palette.** Both slide to `--nimbus-left-palette-clear-x` (446) when a left palette is open, which is exactly where the Timelapse palette sits (444–744) — they were drawn underneath it. `timelapse-palette-open` on `<v-app>` re-anchors them to the top-right, offset by `--nimbus-right-palette-clear-x` when `object-browser-open` says the Object Browser holds that edge. Update both CSS vars together with the palette geometry they are derived from. Verified live: overlap true before, false after.
