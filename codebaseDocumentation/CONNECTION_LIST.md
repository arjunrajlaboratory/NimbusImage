...
- **"Also hide these tracks' objects in the image"** extends the filter to
  ...
  click opens the Object Browser — unless an Analysis gate is also active:
  those two are mutually-evicting right-zone primaries, so Analysis wins the
  click and the tooltip names only what actually opens. The HUD's "passing
