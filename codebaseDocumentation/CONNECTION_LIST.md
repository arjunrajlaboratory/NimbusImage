...
- [ ] **Framing a track moves XY/Z but leaves Time unless it is out of range.** A track on another XY/Z is not drawn at all, so those must follow. Time is the timelapse window's centre and the user scrubs it deliberately; it is clamped to the nearest end of the track's range only when the current frame is outside it. — *"leaves Time alone when it is already inside the track's range"*, *"clamps Time to the nearest end when it is outside the range"*
