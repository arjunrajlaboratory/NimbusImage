...
- [ ] **Every colouring input is in the timelapse watch list.** Track colour is baked into each line feature at draw time and there is no restyle-in-place path for it (unlike hover), so a field of `@/store/timelapse` missing from that `watch([...])` in `AnnotationViewer.vue` changes nothing until an unrelated redraw. Silent: tsc, lint and every draw-path test stay green. — *"rebuilds the timelapse layer when timelapseTrackColoring changes"*, *"…when timelapseColorSeed changes"*
...
- [ ] **Time moves only when NO member is actually drawn, tested against the same window the draw path uses.** `drawTimelapseConnectionsAndCentroids` filters to `[time − modeWindow, time + modeWindow]`, so any check that looks at something else disagrees with what is on screen. Testing the track's overall RANGE failed twice: with the mode off (one frame drawn, members at T1/T5 viewed at T3 — neither drawn) and for a sparse T1→T100 track viewed at T50 with the default window of 10 (T50 is inside [1, 100], every member outside the window). One rule, with the half-width being `modeWindow` in the mode and 0 outside it, covers both — and it also stops the gratuitous moves the range rule made, e.g. at T0 with members at T1/T5 and a window of 10 nothing needs to move. — *"snaps Time when every member is outside the drawn window"*, *"leaves Time alone when a wide window reaches a member"*, *"snaps Time to the nearest end when the window reaches no member"*
- [ ] **Framing a track moves XY/Z but treats Time differently per MODE.** A track on another XY/Z is not drawn at all, so those must follow. In timelapse mode a whole window of frames is drawn, so Time is the window's centre the user scrubs deliberately — leave it, clamping to the nearest end only when the frame is outside the track's range. With the mode OFF only one frame is drawn, and the By-track view is still reachable, so leaving Time alone frames a region containing nothing (members at T1/T5 viewed at T3 shows no member and no link); snap to the nearest member instead. — *"leaves Time alone when it is already inside the track's range"*, *"clamps Time to the nearest end when it is outside the range"*, *"snaps Time to the nearest member outside timelapse mode"*, *"does not move Time outside the mode when already on a member"*
...
- [ ] **A time jump keeps its track's colour.** It was forced to `#ff6b6b`, which broke both colouring controls: "uniform" left those segments red among white ones, and per-track showed a hue swatch against a red line for any track whose drawn segments are all jumps. The dash (`[5, 5]`) and the reduced opacity (0.7) are two cues no other segment has, so the colour was the redundant third one — dropping it makes "the swatch matches the line" true unconditionally. — *"keeps a time-jump segment on the uniform track colour"*, *"…on the track track colour"*
- [ ] **Framing anchors on ONE member, then derives everything from its slice.** A track is not guaranteed to sit on a single XY/Z — `Connect selected` chains by ascending time with no slice constraint — so taking XY/Z from one member while computing the bounding box and time from all of them navigates to one slice and frames another, landing on a frame where nothing is drawn. Pick the anchor (nearest the current frame), then filter members to its slice. — *"frames only the anchor slice for a cross-slice track"*, *"keeps Time inside the anchor slice's range in timelapse mode"*
