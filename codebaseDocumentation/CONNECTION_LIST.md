...
- [ ] **Every throttled/debounced callback is cancelled in `onBeforeUnmount`.** A trailing fire after teardown runs against a dead GeoJS view. The test that guards this *discovers* the throttles from the exposed surface rather than naming them — the version that named the five that existed when it was written stayed green while two more shipped uncancelled. — *"cancels every pending debounced/throttled callback so none fire after teardown"*
