...
- Invalidation of PURE ids is tracked PER PLOT
  ...
  counter moves"*
- The plot-creation tool never reports a plot that does not exist, checked at
  BOTH ends of the work: after insertion, and again after the resolution
  wait, since the user can delete the plot during the seconds those take —
  *"never reports a plot that was removed while its gate resolved"*
...
- The refusal banner names a PARTIAL failure. Resolution is per plot, so a
  ...
  behaviour and misstated what was on screen. Asserted on rendered text,
  since a sentence is what was wrong and nothing typechecks one —
  *"describes a gate refusal as partial, not as everything being
  unfiltered"*
