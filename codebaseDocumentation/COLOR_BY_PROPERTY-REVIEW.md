...
| M15 | P2 (Codex on `74aa2b85`) | `index.ts` `syncConfiguration` | Dispatched `ressourceChanged` with `this.configuration.id` read AFTER the awaited PUT, so a mid-PUT configuration switch invalidated the newly opened configuration's cache instead of the written one's — switching back in-session restored an obsolete legend from the stale cache (and a mid-PUT close would dereference null). Pre-existing master shape, made consequential by the legend paths' reliance on the cache being invalidated on every write. | fixed — the id is captured before the await. Test: mid-PUT-switch case in `colorByProperty.test.ts`, watched failing pre-fix |
| M16 | P2 (Codex on `74aa2b85`) | `models/annotation.py` `_colorContinuous` | Individually finite bounds can overflow their difference (`1e308 - -1e308 == inf`), so `t` computed against infinity — near-zero values silently mapped to the first color and a value at the bound produced NaN, surfacing an internal ValueError as a 400. | fixed — a non-finite resolved span raises a clear ValueError before any write. Test: `testOverflowingSpanIsAClean400BeforeAnyWrite` (MongoDB-backed, runs in CI) |
| M17 | P3 (Codex on `74aa2b85`) | `COLOR_BY_PROPERTY.md` | Three places claimed a 256-category cap; `MAX_CATEGORIES` is derived from the palette (20 × 5 = 100), so callers would expect 101–256 categories to succeed. | fixed — all three now state the derived 100 and mark the 256 in the measurement narrative as the then-current cap |
| M18 | P2 (Codex on `a8512c07`) | `index.ts` `saveColorByPropertyFor` | A configuration REOPENED while the direct PUT was in flight kept its pre-write live copy — the cache eviction only helps the next load — so the session showed a legend the backend no longer had. | fixed — after the PUT, if the captured configuration has become current again, its live slot is patched via the mutation. Test: reopened-during-PUT case in `colorByProperty.test.ts`, watched failing pre-fix |
| M19 | P2 (Codex on `a8512c07`) | `models/annotation.py` `colorByProperty` | M11's membership filter chunked its $in at COLOR_WRITE_CHUNK, issuing ~15 sequential queries on the 708K dataset before any coloring started — the looped-DB-calls shape the repo's top review rule forbids. | fixed — one dataset-scoped indexed id scan. Test: `testMembershipFilterIsOneQueryNotAChunkLoop` pins the query count with a monkeypatched chunk size of 1 |
| M20 | P2 (Codex on `8f8fc4bd`) | `annotation.ts` failure predicates | 401/403 were classified as "may have written", but the endpoints check dataset WRITE access before any mutation — a READ-only user clicking Apply lost their valid legend and paid a full refetch for a guaranteed no-write rejection. | fixed — shared `failureMayHaveWritten` treats 400/401/403 as pre-write across apply, clear, and manual paths. Tests: 403 apply case + [400,401,403] manual loop, watched failing pre-fix |
| M21 | P2 (Codex on `8f8fc4bd`) | `index.ts` `saveColorByPropertyFor` | Two overlapping direct writes for different datasets on one non-current configuration cloned the same cached map; the later full-key PUT erased the earlier's slot — a lost write. | fixed — direct read-modify-writes are serialized per configuration (file-scope promise chain, read inside the chained task so each write sees its predecessor's result). Test: concurrent-saves case, watched failing pre-fix |
| M22 | P2 (Codex on `8f8fc4bd`) | `index.ts` `saveColorByPropertyFor` | M18's post-PUT live patch could clobber a NEWER legend written after the configuration was reopened mid-PUT. | fixed — the live patch applies only while the slot still holds the value the write was based on. Test: newer-legend case, watched failing pre-fix |
| M23 | P2 (Codex on `c031d5a2`) | `index.ts` `saveColorByPropertyFor` | M21 chained only the direct writes — the one-of-two-symmetric-paths miss: a newer current-path write (recolor after reopening, or the legend collapse toggle via `saveColorByProperty`) completing during a direct write's awaited read was erased on the backend by the older full-key PUT landing last. | fixed — every legend write joins the per-configuration chain (`saveColorByProperty` delegates to `saveColorByPropertyFor`; the current-vs-direct decision moves inside the chained task, made at execution time). Test: queued current-path write case, watched failing pre-fix |
