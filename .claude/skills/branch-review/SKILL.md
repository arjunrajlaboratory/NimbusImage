...
- Counts/limits clamped to a `MAX_*` constant — unauthenticated callers must not be able to force unbounded DB or serialization work. **Trace the arithmetic, don't just confirm a `MAX_*` exists.** Substitute `-1`, `0`, and `MAX+1` by hand and check what reaches PyMongo. `-1` is the case that hides: `min(limit or MAX, MAX)` preserves `-1`, and if the handler then reads `limit + 1` (the standard "one extra row for `hasMore`" trick) that becomes `0` — Girder's *unlimited* sentinel — so the cap is bypassed entirely and `documents[:-1]` returns almost everything. `0` and `MAX+1` both behave correctly, so a spot-check passes while the bypass ships (Codex P1, PR #1278). The correct clamp is `min(max(1, limit or MAX), MAX)`, plus `offset = max(0, offset)`.
- **`autoDescribeRoute` is not a substitute for the validation helpers.** Girder coerces `.pagingParams()` ints (clean 400 on non-numeric) but passes negatives through, and a plain `.param('someId', ...)` arrives as a raw string with no ObjectId validation at all. An endpoint can look framework-validated and still 500 on `?someId=nope`. Check `autoDescribeRoute` endpoints against the helpers exactly as you would a `getBodyJson()` endpoint.
- Sort keys are caller input: `.pagingParams()` accepts any field name. A clamped `limit` with a free-form `sort` still lets a public caller force a blocking sort over every accessible document. Watch for this right after a diff adds an index — the index usually covers only the *default* sort.
- When one endpoint has the gap, check sibling public endpoints in the same file — this pattern recurs in clusters. A sibling that raises bare `ValueError` for a missing field is the same bug wearing a layer-violation costume: `ValueError` from an API method is a 500.
- **Sweep the sibling on every dimension, not just the one the diff touched.** *Partial* hardening reads as hardening: in PR #1278 the diff gave `by_folders` full `folderIds` shape validation **and** `max(0, offset)`, but left `limit` unclamped and `sort` free-form — while that endpoint returns *whole* documents (`meta` included), making it the heavier of the two. The `offset` fix is the tell: someone thought about paging hygiene and stopped halfway. When a diff hardens endpoint A and touches sibling B, check B against A's *full* checklist (validation shape, limit clamp, sort allowlist, offset floor), and prefer extracting shared helpers so the two cannot drift again. Also check whether the response weight differs — the endpoint returning fat documents needs the clamp *more*, not less.
